...
export default function cleanTranscriptHTML(html) {
    if (!html)
        return "";
    ...
    while (/<li[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/li>/gis.test(decoded)) {
        decoded = decoded.replace(/<li[^>]*>\s*<p[^>]*>(.*?)<\/p>\s*<\/li>/gis, "<li>$1</li>");
    }
    // 🔁 Bersihkan <li> yang mengandung <p><strong>...</strong></p> (opsional tambahan)
    while (/<li[^>]*>\s*<p[^>]*>(<strong[^>]*>.*?<\/strong>.*?)<\/p>\s*<\/li>/gis.test(decoded)) {
        decoded = decoded.replace(/<li[^>]*>\s*<p[^>]*>(<strong[^>]*>.*?<\/strong>.*?)<\/p>\s*<\/li>/gis, "<li>$1</li>");
    }
    // <p> bungkus <ul>/<ol>/<li>
