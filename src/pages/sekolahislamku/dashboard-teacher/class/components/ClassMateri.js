...
import ParentTopBar from "@/pages/sekolahislamku/components/home/ParentTopBar";
import ParentSidebar from "@/pages/sekolahislamku/components/home/ParentSideBar";
import { CalendarDays, Download, ArrowLeft, Plus, Search, Filter, ClipboardList, Timer, BarChart2, Pencil, Trash2, Eye, } from "lucide-react";
/* ====== Helpers tanggal ====== */
const atLocalNoon = (d) => {
    const x = new Date(d);
    x.setHours(12, 0, 0, 0);
...
const QK = {
    ...
    MATERIALS: (classId) => ["teacher-class-materials", classId],
    QUIZZES: (classId) => ["teacher-class-quizzes", classId], // NEW
};
/* ====== Dummy fetch ====== */
...
async function fetchMaterialsByClass(classId) {
    const now = new Date().toISOString();
    const yesterday = new Date(Date.now() - 864e5).toISOString();
    const base = {
        "tpa-a": [
            {
                id: "m-001",
                title: "Mad Thabi'i — Ringkasan & Contoh",
                description: "Materi pokok tentang mad thabi'i: definisi, cara membaca, dan contoh.",
                type: "pdf",
                createdAt: yesterday,
                attachments: [{ name: "mad-thabii.pdf" }],
                author: "Ustadz Abdullah",
...
async function fetchQuizzesByClass(classId) {
    const now = new Date();
...
export default function ClassMateri() {
    ...
    const filtered = useMemo(() => {
        ...
        const qq = q.trim().toLowerCase();
        if (qq) {
            list = list.filter((m) => m.title.toLowerCase().includes(qq) ||
                (m.description ?? "").toLowerCase().includes(qq));
        }
        return [...list].sort((a, b) => +new Date(b.createdAt) - +new Date(a.createdAt));
    }, [materials, q, type]);
    const todayISO = toLocalNoonISO(new Date());
    const { slug, id: classId } = useParams();
    /* ====== Actions: Materials ====== */
    ...
    const handleAddQuiz = async () => {
      ...
      `,
            ...
            preConfirm: () => {
                ...
                const dueRaw = document.getElementById("quiz-due")
                    ?.value;
                const durationMin = parseInt(document.getElementById("quiz-duration")
                    ?.value || "0", 10);
                const totalQuestions = parseInt(document.getElementById("quiz-questions")
                    ?.value || "0", 10);
                if (!title) {
        ...
        qc.setQueryData(QK.QUIZZES(id), (old = []) => [newQuiz, ...old]);
        await Swal.fire({
    ...
    const handleEditQuiz = async (qz) => {
        ...
        <input id="quiz-due" type="datetime-local" class="swal2-input" value="${qz.dueAt ? new Date(qz.dueAt).toISOString().slice(0, 16) : ""}" />
        <input id="quiz-duration" type="number" min="1" class="swal2-input" placeholder="Durasi (menit)" value="${qz.durationMin ?? ""}" />
        <input id="quiz-questions" type="number" min="1" class="swal2-input" placeholder="Jumlah soal" value="${qz.totalQuestions}" />
      `,
            focusConfirm: false,
            showCancelButton: true,
            confirmButtonText: "Simpan",
        ...
        if (!isConfirmed || !payload)
            return;
        qc.setQueryData(QK.QUIZZES(id), (old = []) => old.map((x) => x.id === qz.id
            ? {
                ...x,
                ...payload,
            }
    ...
    const handleTogglePublish = async (qz) => {
        if (!qz.published) {
        ...
        qc.setQueryData(QK.QUIZZES(id), (old = []) => old.map((x) => (x.id === qz.id ? { ...x, published: !x.published } : x)));
    };
    const handleDeleteQuiz = async (qz) => {
        ...
        if (!res.isConfirmed)
            return;
        qc.setQueryData(QK.QUIZZES(id), (old = []) => old.filter((x) => x.id !== qz.id));
        await Swal.fire({
            title: "Terhapus",
            icon: "success",
            timer: 1000,
    ...
    const handleViewResults = async (qz) => {
        await Swal.fire({
    ...
    const [openModal, setOpenModal] = useState(false);
    const handleAddMateri = async (payload) => {
        try {
            const nowISO = new Date().toISOString();
            const newItem = {
                id: `m-${Date.now()}`,
            ...
            qc.setQueryData(QK.MATERIALS(id), (old = []) => [
                newItem,
                ...old,
            ]);
            qc.setQueryData(QK.CLASSES, (old = []) => old.map((c) => c.id === id
                ? { ...c, materialsCount: (c.materialsCount ?? 0) + 1 }
                : c));
            setOpenModal(false);
            await Swal.fire({
    ...
    const handleEditMateri = async (payload) => {
        try {
            const nowISO = new Date().toISOString();
            qc.setQueryData(QK.MATERIALS(id), (old = []) => old.map((m) => m.id === payload.id
                ? {
                    ...m,
                    title: payload.title,
                    description: payload.description,
    ...
    const handleDelete = async (m) => {
        ...
        if (!res.isConfirmed)
            return;
        qc.setQueryData(QK.MATERIALS(id), (old = []) => old.filter((x) => x.id !== m.id));
        await Swal.fire({
            title: "Terhapus",
            text: "Materi berhasil dihapus.",
            icon: "success",
    ...
    const ModalAddClassMateri = (_) => null;
    const ModalEditClassMateri = (_) => null;
    return (_jsxs("div", { className: "min-h-screen w-full", style: { background: palette.white2, color: palette.black1 }, children: [_jsx(ParentTopBar, { palette: palette, title: cls ? `Materi: ${cls.name}` : "Materi Kelas", gregorianDate: todayISO, hijriDate: hijriLong(todayISO), showBack: true }), _jsx("main", { className: "w-full px-4 md:px-6  md:py-8", children: _jsxs("div", { className: "max-w-screen-2xl mx-auto flex flex-col lg:flex-row gap-4 lg:gap-6", children: [_jsx("aside", { className: "w-full lg:w-64 xl:w-72 flex-shrink-0", children: _jsx(ParentSidebar, { palette: palette }) }), _jsxs("div", { className: "flex-1 flex flex-col space-y-6 min-w-0", children: [_jsxs("div", { className: "md:flex hidden gap-3 items-center ", children: [_jsx(Btn, { palette: palette, variant: "ghost", onClick: () => navigate(-1), className: "gap-1 ", children: _jsx(ArrowLeft, { size: 20 }) }), _jsx("h1", { className: "textlg font-semibold", children: "Materi" })] }), _jsx(SectionCard, { palette: palette, children: _jsxs("div", { className: "p-4 md:p-5 flex items-start justify-between gap-4", children: [_jsxs("div", { className: "min-w-0", children: [_jsx("div", { className: "text-lg md:text-xl font-semibold", children: cls?.name ?? "—" }), _jsxs("div", { className: "mt-1 flex flex-wrap items-center gap-2 text-sm", style: { color: palette.black2 }, children: [_jsx(Badge, { variant: "outline", palette: palette, children: _jsx("span", { style: { color: palette.black2 }, children: cls?.room ?? "-" }) }), _jsxs("span", { children: ["Wali Kelas: ", cls?.homeroom ?? "-"] }), _jsxs("span", { children: ["\u2022 ", cls?.academicTerm ?? "-"] }), _jsxs("span", { children: ["\u2022 Angkatan ", cls?.cohortYear ?? "-"] })] })] }), _jsx("div", { className: "flex items-center gap-2", children: _jsxs(Btn, { palette: palette, variant: "ghost", size: "sm", onClick: () => setOpenModal(true), children: [_jsx(Plus, { size: 16, className: "mr-1" }), "Tambah Materi"] }) })] }) }), _jsx(SectionCard, { palette: palette, children: _jsxs("div", { className: "p-4 md:p-5 grid grid-cols-1 md:grid-cols-3 gap-3", children: [_jsxs("div", { className: "flex items-center gap-2 rounded-xl border px-3 h-10", style: {
                                                    borderColor: palette.silver1,
                                                    background: palette.white1,
                                                }, children: [_jsx(Search, { size: 16 }), _jsx("input", { value: q, onChange: (e) => setQ(e.target.value), placeholder: "Cari materi\u2026", className: "bg-transparent outline-none text-sm w-full", style: { color: palette.black1 } })] }), _jsxs("div", { className: "flex items-center gap-2 rounded-xl border px-3 h-10", style: {
                                                    borderColor: palette.silver1,
                                                    background: palette.white1,
                                                }, children: [_jsx(Filter, { size: 16 }), _jsxs("select", { value: type, onChange: (e) => setType(e.target.value), className: "bg-transparent outline-none text-sm w-full", style: { color: palette.black1 }, children: [_jsx("option", { value: "all", children: "Semua tipe" }), _jsx("option", { value: "pdf", children: "PDF" }), _jsx("option", { value: "doc", children: "Dokumen" }), _jsx("option", { value: "ppt", children: "Presentasi" }), _jsx("option", { value: "video", children: "Video" }), _jsx("option", { value: "link", children: "Tautan" })] })] })] }) }), _jsxs("div", { className: "grid gap-3", children: [isFetching && filtered.length === 0 && (_jsx(SectionCard, { palette: palette, children: _jsx("div", { className: "p-6 text-sm", style: { color: palette.silver2 }, children: "Memuat materi\u2026" }) })), filtered.map((m) => (_jsxs(SectionCard, { palette: palette, className: "p-0 hover:shadow-sm transition-shadow", style: { background: palette.white1 }, children: [_jsx("div", { className: "p-4 md:p-5", children: _jsx("div", { className: "flex items-start justify-between gap-4", children: _jsxs("div", { className: "min-w-0", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "text-base font-semibold truncate", children: _jsx("span", { style: { color: palette.black2 }, children: m.title }) }), _jsx(Badge, { palette: palette, variant: m.type === "pdf"
                                                                                ? "secondary"
                                                                                : m.type === "doc"
                                                                                    ? "info"
                                                                                    : m.type === "ppt"
                                                                                        ...
                                                                                        : m.type === "video"
                                                                                            ? "success"
                                                                                            : "outline", className: "h-6", children: m.type.toUpperCase() })] }), m.description && (_jsx("p", { className: "text-sm mt-1 line-clamp-2", style: { color: palette.black2 }, children: m.description })), _jsxs("div", { className: "mt-2 flex flex-wrap items-center gap-2 text-sm", style: { color: palette.black2 }, children: [_jsx(CalendarDays, { size: 14 }), _jsxs("span", { children: ["Dibuat: ", dateLong(m.createdAt)] }), m.updatedAt && (_jsxs("span", { children: ["\u2022 Diperbarui: ", dateLong(m.updatedAt)] })), m.author && _jsxs("span", { children: ["\u2022 Oleh ", m.author] }), m.attachments?.length ? (_jsxs(_Fragment, { children: [_jsx("span", { children: "\u2022" }), _jsxs("span", { children: [m.attachments.length, " lampiran"] })] })) : null] })] }) }) }), _jsxs("div", { className: "px-4 md:px-5 pb-4 md:pb-5 pt-3 border-t flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3", style: { borderColor: palette.silver1 }, children: [_jsx("div", { className: "text-sm", style: { color: palette.black2 }, children: "Aksi cepat untuk materi ini" }), _jsxs("div", { className: "flex gap-2", children: [_jsx(Btn, { palette: palette, size: "sm", variant: "outline", onClick: () => handleDownload(m), children: _jsx(Download, { size: 16, className: "mr-1" }) }), _jsx(Btn, { palette: palette, size: "sm", onClick: () => {
                                                                        setEditingItem(m);
                                                                        setOpenEdit(true);
                                                                    }, children: _jsx(Pencil, { className: "mr-1", size: 16 }) }), _jsx(Btn, { palette: palette, size: "sm", variant: "destructive", onClick: () => handleDelete(m), children: _jsx(Trash2, { className: "mr-1", size: 16 }) })] })] })] }, m.id))), filtered.length === 0 && !isFetching && (_jsx(SectionCard, { palette: palette, children: _jsx("div", { className: "p-6 text-sm text-center", style: { color: palette.silver2 }, children: "Belum ada materi untuk kelas ini." }) }))] }), _jsx(SectionCard, { palette: palette, children: _jsxs("div", { className: "p-4 md:p-5 space-y-4", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(ClipboardList, { size: 18, style: { color: palette.primary } }), _jsx("h2", { className: "font-semibold", style: { color: palette.black1 }, children: "Quiz Kelas" })] }), _jsx(Btn, { palette: palette, size: "sm", onClick: handleAddQuiz, children: _jsx(Plus, {}) })] }), isFetchingQuiz && (_jsx("div", { className: "text-sm", style: { color: palette.silver2 }, children: "Memuat quiz\u2026" })), !isFetchingQuiz && quizzes.length === 0 && (_jsx("div", { className: "text-sm text-center", style: { color: palette.silver2 }, children: "Belum ada quiz untuk kelas ini." })), _jsx("div", { className: "grid gap-3", children: quizzes.map((qz) => (_jsxs(SectionCard, { palette: palette, className: "p-0", children: [_jsx("div", { className: "p-4 md:p-5", children: _jsx("div", { className: "flex items-start justify-between gap-4", children: _jsxs("div", { className: "min-w-0", children: [_jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [_jsx("span", { className: "font-semibold text-base", style: { color: palette.black2 }, children: qz.title }), _jsx(Badge, { palette: palette, variant: qz.published ? "success" : "outline", className: "h-6", children: qz.published ? "Terpublikasi" : "Draft" })] }), qz.description && (_jsx("p", { className: "text-sm mt-1", style: { color: palette.black2 }, children: qz.description })), _jsxs("div", { className: "mt-2 flex flex-wrap items-center gap-3 text-sm", style: { color: palette.black2 }, children: [_jsx(Timer, { size: 14 }), _jsxs("span", { children: [qz.durationMin ?? 0, " menit"] }), _jsxs("span", { children: ["\u2022 ", qz.totalQuestions, " soal"] }), _jsxs("span", { children: ["\u2022 Deadline: ", dateLong(qz.dueAt)] }), _jsxs("span", { children: ["\u2022 Attempt: ", qz.attempts] }), _jsxs("span", { children: ["\u2022 Rata-rata: ", qz.avgScore ?? 0] })] })] }) }) }), _jsxs("div", { className: "px-4 md:px-5 pb-4 md:pb-5 pt-3 border-t flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3", style: { borderColor: palette.silver1 }, children: [_jsx("div", { className: "text-sm", style: { color: palette.black2 }, children: "Aksi quiz" }), _jsxs("div", { className: "flex gap-2 flex-wrap", children: [_jsx(Btn, { palette: palette, size: "sm", variant: "outline", onClick: () => handleTogglePublish(qz), children: qz.published ? "Tarik Publikasi" : "Publikasikan" }), _jsx(Btn, { palette: palette, size: "sm", onClick: () => navigate(`/${slug}/guru/quizClass/${qz.id}`), children: _jsx(Eye, { size: 16, className: "mr-1" }) }), _jsx(Btn, { palette: palette, size: "sm", onClick: () => handleEditQuiz(qz), children: _jsx(Pencil, { size: 16, className: "mr-1" }) }), _jsx(Btn, { palette: palette, size: "sm", variant: "outline", onClick: () => handleViewResults(qz), children: _jsx(BarChart2, { size: 16, className: "mr-1" }) }), _jsx(Btn, { palette: palette, size: "sm", variant: "destructive", onClick: () => handleDeleteQuiz(qz), children: _jsx(Trash2, { size: 16, className: "mr-1" }) })] })] })] }, qz.id))) })] }) })] })] }) })] }));
}
