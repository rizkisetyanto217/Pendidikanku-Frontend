...
export default function DKMAddEditFullTransciptLectureSessions() {
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            ...
            if (materialData?.lecture_sessions_material_id) {
                return axios.put(`/api/a/lecture-sessions-materials/${materialData.lecture_sessions_material_id}`, payload);
            }
            else {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsx(PageHeader, { title: materialData ? "Edit Materi Lengkap" : "Tambah Materi Lengkap", onBackClick: () => history.back() }), _jsxs("div", { className: "p-4 rounded-2xl shadow-sm", style: { backgroundColor: theme.white1 }, children: [_jsx(RichEditor, { value: content, onChange: setContent, placeholder: "Tulis materi kajian secara lengkap...", label: "Materi Lengkap" }), _jsx("div", { className: "mt-6 text-right", children: _jsx("button", { onClick: () => mutation.mutate(), disabled: mutation.isPending, className: "px-6 py-2 rounded-lg font-semibold", style: {
                                backgroundColor: theme.primary,
                                color: theme.white1,
                                opacity: mutation.isPending ? 0.6 : 1,
                            }, children: mutation.isPending ? "Menyimpan..." : "Simpan" }) })] })] }));
}
