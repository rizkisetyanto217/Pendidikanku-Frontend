...
export default function MasjidExamLecture() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await axios.get(`/public/lecture-exams/${id}/questions/by-lecture`);
            return res.data;
        },
        enabled: !!id,
        staleTime: 5 * 60 * 1000,
    ...
    if (!data || !data.questions?.length) {
        return (_jsxs("div", { className: "p-4 pb-28", children: [_jsx(PageHeaderUser, { title: "Latihan Soal", onBackClick: () => navigate(-1) }), _jsx("div", { className: "mt-4 text-sm text-center text-gray-500 dark:text-white/70", children: "Belum ada soal tersedia untuk sesi ini." })] }));
    }
    ...
    return (_jsxs("div", { className: "p-4 max-w-2xl mx-auto", children: [_jsx(PageHeaderUser, { title: "Latihan Soal", onBackClick: () => navigate(-1) }), _jsx("div", { className: "relative h-2 rounded-full bg-gray-200 dark:bg-white/10 mb-6 mt-2", children: _jsx("div", { className: "absolute top-0 left-0 h-full rounded-full transition-all duration-300", style: {
                        width: `${(progressCount / (data?.questions?.length || 1)) * 100}%`,
                        backgroundColor: theme.primary,
                    } }) }), _jsx("p", { className: "text-sm font-medium mb-4", style: { color: theme.black1 }, children: question.lecture_sessions_question }), _jsx("div", { className: "space-y-3 mb-6", children: question.lecture_sessions_question_answers.map((opt) => {
                    const isSel = selected === opt;
                    const isUserAnswerCorrect = selected?.startsWith(question.lecture_sessions_question_correct);
                ...
                }) }), showAnswer && isCorrect && (_jsxs("div", { className: "text-sm mb-4 p-3 rounded", style: { backgroundColor: theme.success2, color: theme.success1 }, children: [_jsx("strong", { children: "\u2705 Jawaban Benar" }), _jsx("br", {}), question.lecture_sessions_question_explanation] })), !showAnswer ? (_jsx("button", { className: "w-full py-3 rounded-lg text-sm font-semibold transition-all", style: {
                    backgroundColor: selected ? theme.primary : theme.silver2,
                    color: theme.white1,
                }, onClick: handleCheck, disabled: !selected, children: "Cek" })) : (_jsx("button", { className: "w-full py-3 mt-3 rounded-lg text-sm font-semibold transition-all", style: { backgroundColor: theme.primary, color: theme.white1 }, onClick: handleNext, children: "Lanjut" }))] }));
}
