...
import { ArrowDownCircle, CalendarDays, Tag, Clock, FileDown, TrendingUp, DollarSign, } from "lucide-react";
// =====================
// Utils
// =====================
...
const StatsGrid = ({ stats, loading, theme }) => {
    const items = [
        ...
        },
        {
            ...
            icon: _jsx(ArrowDownCircle, { size: 20 }),
        },
        {
...
const IncomeList = ({ transactions, loading, error, theme }) => {
    ...
    if (loading)
        return (_jsx("p", { className: "text-center py-8", style: { color: theme.silver2 }, children: "Memuat data pemasukan\u2026" }));
    if (error)
        return (_jsx("p", { className: "text-center py-8", style: { color: theme.error1 }, children: "Gagal memuat data pemasukan." }));
    if (!transactions.length) {
        return (_jsxs("div", { className: "text-center py-8", children: [_jsx(ArrowDownCircle, { size: 48, className: "mx-auto mb-3 opacity-50", style: { color: theme.silver2 } }), _jsx("p", { className: "text-sm", style: { color: theme.silver2 }, children: "Belum ada pemasukan untuk bulan ini." })] }));
    }
...
const PemasukanPage = () => {
    ...
    const { data: statsResp, isLoading: loadingStats } = useQuery({
        ...
        queryFn: async () => (await axios.get(`/public/masjids/${slug}/financials/income/stats`, {
            params: { month },
        })).data,
        enabled: !!slug && !!month,
        staleTime: 120000,
    });
    const { data: incomeResp, isLoading: loadingIncome, isError: errorIncome, } = useQuery({
        queryKey: ["income-transactions", slug, month],
        queryFn: async () => (await axios.get(`/public/masjids/${slug}/financials/income`, {
            params: { month },
        })).data,
        enabled: !!slug && !!month,
        staleTime: 120000,
    ...
    return (_jsxs("div", { className: "pb-24 max-w-2xl mx-auto", children: [_jsx(PageHeaderUser, { title: "Data Pemasukan", onBackClick: () => navigate(-1), withPaddingTop: true }), _jsx(MonthFilter, { month: month, onMonthChange: onMonthChange, onExport: () => exportToCSV(transactions, slug, month), theme: theme }), _jsx(StatsGrid, { stats: statsResp?.data, loading: loadingStats, theme: theme }), _jsxs("section", { className: "mt-6", children: [_jsxs("header", { className: "flex items-center gap-2 mb-4", children: [_jsx(ArrowDownCircle, { size: 20, style: { color: theme.success1 } }), _jsx("h2", { className: "text-lg font-semibold", style: { color: theme.black1 }, children: "Riwayat Pemasukan" })] }), _jsx(IncomeList, { transactions: transactions, loading: loadingIncome, error: errorIncome, theme: theme })] }), _jsx(BottomNavbar, {})] }));
};
