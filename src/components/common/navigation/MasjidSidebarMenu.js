...
export default function MasjidSidebarMenu({ menus, title = "Beranda", }) {
    const location = useLocation();
    const { isDark, themeName } = useHtmlDarkMode();
    const theme = pickTheme(themeName, isDark);
    return (_jsxs("div", { className: "hidden md:block w-48 rounded-xl p-4 shadow-sm transition-colors", style: {
